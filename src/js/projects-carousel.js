  ...
  async function load() {
    try {
      ...
      const card = (p, i) => {
        ...
        const hoverBg     = tone === 'primary' ? 'rgba(139, 92, 246, 0.1)' : 'rgba(74, 222, 128, 0.1)';
        ...
        return `
<div class="flex-none w-full p-2 md:w-1/2 lg:w-1/3 snap-center">
  <div class="relative flex flex-col h-full overflow-hidden transition-all duration-300 transform border rounded-xl group bg-white/5 border-white/10 hover:-translate-y-1 ${hoverShadow}">
    <div class="absolute inset-0 z-0 transition-opacity duration-300 opacity-0 group-hover:opacity-100" style="background: radial-gradient(circle at center, transparent 40%, ${hoverBg});"></div>
    <div class="absolute inset-0 transition-all duration-300 border-2 border-transparent rounded-xl pointer-events-none ${hoverRing}"></div>
    <div class="relative z-10 flex flex-col flex-grow">
      <div class="w-full h-48 bg-center bg-cover" role="img" aria-label="${p.title} cover" style="background-image: url('${img}')"></div>
      <div class="flex flex-col flex-grow gap-4 p-6">
        ...
        <div class="flex gap-4">${repoLink}${liveLink}</div>
      </div>
    </div>
  </div>
</div>`;
      ...
      carousel.innerHTML = items.map(card).join('');

      // avisa o Alpine para recomputar os bullets e o tamanho da lista
